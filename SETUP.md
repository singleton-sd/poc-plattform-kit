...
App registration: `ssd-pocpk-gha-oidc-dev` with federated credentials. Prefer **ID-form** subjects (`repo:ORG@ORG_ID/REPO@REPO_ID:pull_request` / `:ref:refs/heads/main`); classic `repo:org/repo:...` subjects may remain for compatibility. **FIC subject must match JWT `sub` exactly.** Roles: **Reader** on RG (marketing SWA preview), **Contributor** on RG (ACA API production + API/web preview deploy), optional **Website Contributor** on `pocpk-api-si5fhs6dvxiha` only while App Service dual-run remains, **Key Vault Secrets User** on `ssd-pocpk-kv-dev-ae` (app runtime + `acr-admin-*`), **Key Vault Secrets User** on `ssd-devtools-kv-prod-ae` (org CI secrets — cross-subscription), **App Configuration Data Reader**. Production ACA Bicep (`infra/container-apps-api-prod.bicep`) also creates MI role assignments on Key Vault + App Configuration — grant the OIDC SP **Role Based Access Control Administrator** (or **User Access Administrator**) scoped to those two resources (Contributor alone cannot `Microsoft.Authorization/roleAssignments/write`), or create those assignments once with a bootstrap Owner identity. ACR push uses OIDC -> KV `acr-admin-*` (not AcrPush / not GitHub Secrets).
