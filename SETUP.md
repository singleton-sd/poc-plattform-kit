...
- [ ] Branch protection on `main` (solo-repo policy Ã¢â‚¬â€ see below)
...
1. Open the repo Ã¢â€ â€™ **Settings** Ã¢â€ â€™ **Rules** Ã¢â€ â€™ **Rulesets** (or classic **Branches**).
...
5. **Human merge only** Ã¢â‚¬â€ agents never merge. AI reviews are **comments only** when the reviewer shares the authorÃ¢â‚¬â„¢s GitHub identity.

### Branch naming (agents + optional GitHub rules)

**Convention (primary Ã¢â‚¬â€ agents follow `AGENTS.md`):**
...
1. Open the repo Ã¢â€ â€™ **Settings** Ã¢â€ â€™ **Rules** Ã¢â€ â€™ **Rulesets**.
...
## 2. ClickUp (workspace `90161394355`) Ã¢â‚¬â€ locked locations
...
- [ ] Connect ClickUp Ã¢â€ â€ Cursor (App Center + Cursor API key); default repo = this GitHub repo

## 3. Agent automations

- [ ] Implementer: pick tickets in **READY FOR AI** Ã¢â€ â€™ **assign self** (`assignees: ["me"]`) Ã¢â€ â€™ **IN PROGRESS** Ã¢â€ â€™ PR Ã¢â€ â€™ **PR hygiene** (CI + mergeable) Ã¢â€ â€™ **READY FOR REVIEW**
- [ ] Reviewer: pick tickets in **READY FOR REVIEW** Ã¢â€ â€™ **assign self** for the review phase (comment prior implementer if they must stay visible) Ã¢â€ â€™ post review **comments** Ã¢â€ â€™ hygiene (mergeable + CI + Bugbot/human feedback) Ã¢â€ â€™ **READY FOR HUMAN**
- [ ] Assignment only when claiming work Ã¢â‚¬â€ not when browsing
- [ ] Humans only: merge PR when **READY FOR HUMAN**, then set **COMPLETE**
- [ ] PR hygiene labels (`needs-rebase`, `ci-failed`, `has-feedback`) from `.github/workflows/pr-hygiene.yml` Ã¢â‚¬â€ see `docs/pr-pipelines.md` / `AGENTS.md`
...
**IaC:** [`infra/`](./infra/) Ã¢â‚¬â€ `powershell -File ./infra/deploy.ps1`

### Locked: cost + naming

- **Cost:** cheapest SKUs that still work Ã¢â‚¬â€ SQL **Basic**, App Service **B1** (custom-domain HTTPS + Nest always-on), SWA **Free** Ãƒâ€”2 (app + marketing), Service Bus **Standard** (topics; not Premium), Key Vault **Standard**, App Configuration **Free**, ACR **Basic**, Container Apps **Consumption** (API PR previews + OpenFGA).
- **Naming (new resources):** CAF `{org}-{app}-{resource}-{env}-{region}` Ã¢â€ â€™ e.g. `ssd-pocpk-kv-dev-ae`, `ssd-pocpk-appcs-dev-ae`, `ssd-pocpk-mkt-dev-ae`. ACR is alphanumeric-only: `ssdpocpkacrdevae`.
- **Legacy live names** (`pocpk-*-si5fhs6dvxiha`) stay as-is (renames recreate). See alias table in [`infra/README.md`](./infra/README.md).

### Custom domains (locked) Ã¢â‚¬â€ DNS in AWS Route53
...
PR / preview URLs stay on Azure defaults (`*.azurestaticapps.net`, ACA preview hostnames) Ã¢â‚¬â€ no custom preview domains.
...
| `app.plattform-kit.poc` | CNAME | web SWA default hostname (e.g. `kind-rock-Ã¢â‚¬Â¦.azurestaticapps.net`) |
...
| App Service Plan + API | `pocpk-plan` / `pocpk-api-si5fhs6dvxiha` | https://api.plattform-kit.poc.singletonsd.com (default: `Ã¢â‚¬Â¦.azurewebsites.net`) | **B1** |
| Static Web App (app) | `pocpk-web-si5fhs6dvxiha` | https://app.plattform-kit.poc.singletonsd.com (default: `Ã¢â‚¬Â¦.azurestaticapps.net`) | Free |
...
| **CI/CD** | GitHub Actions **OIDC** Ã¢â€ â€™ Azure | Workflows log in with federated creds, then `az keyvault secret show` / App Config at **job runtime**. |

| Layer | Choice |
| --- | --- |
| AuthN + coarse roles | Entra via **SingleSignOn** (e.g. tenant-admin, support-agent) |
| Fine-grained authZ | **Permissions** pillar Ã¢â‚¬â€ `Check(subject, action, resource)` |
...
Other pillars call Permissions (sync HTTP or cache); never embed authZ rules in Contact/etc. Optional permission-denial events Ã¢â€ â€™ Audit.
...
*(Later after Entra: `auth-secret`, `azure-ad-client-secret`, Ã¢â‚¬Â¦)*

**Telemetry:** shared App Insights + LAW Ã¢â‚¬â€ see [`docs/telemetry.md`](docs/telemetry.md) / ClickUp Architecture Doc.

**GitHub Actions Ã¢â‚¬â€ allowed identifiers only (repository Variables, not Secrets):**
...
| GitHub Actions | OIDC Ã¢â€ â€™ Azure login Ã¢â€ â€™ Key Vault / App Config at runtime. |
...
- [x] Core deploy succeeded Ã¢â‚¬â€ SQL, App Service, SWA Free, Service Bus Standard
...
- [x] ClickUp ops list custom fields: **Claim Token** (`50a8d70c-e3a6-4bd7-8e3d-7661eaf6e6c7`), **Preview URL**, **Token Estimate**, **Token Spent** â€” UUIDs cached in `AGENTS.md`
...
- [x] Route53 CNAMEs/TXT (AWS) Ã¢â‚¬â€ applied; re-run via `scripts/apply-route53-dns.ps1`
- [x] Custom domains + managed certs bound Ã¢â‚¬â€ re-run via `scripts/bind-custom-domains.ps1`
- [x] Reusable domain config `infra/custom-domains.pocpk.json` (+ schema) for other products
- [ ] Entra app registration (SPA + API) Ã¢â‚¬â€ secrets in KV; config keys in App Config
- [ ] API `/health` on custom domain returns 200 (currently 503 Ã¢â‚¬â€ app runtime, not DNS/TLS)
- [ ] Tighten SQL firewall (`AllowAllDevPoC` Ã¢â€ â€™ your IP)
...
- [ ] ~~S1 slots for API PR previews~~ Ã¢â‚¬â€ **deprecated**; use Container Apps Path B
...
4. If login fails with consent errors: Entra admin grants the enterprise app access (tenant admin consent for the SP) Ã¢â‚¬â€ Portal Ã¢â€ â€™ Enterprise applications Ã¢â€ â€™ `ssd-pocpk-gha-oidc-dev` Ã¢â€ â€™ Permissions / admin consent, or re-run role assignments as subscription Owner.
5. SWA deploy token lives only in KV as `swa-deployment-token` (`az staticwebapp secrets list` Ã¢â€ â€™ `az keyvault secret set`).
...
| `preview-web.yml` | `apps/web/**`, `packages/**` | SWA **PR preview** via OIDC Ã¢â€ â€™ KV token |
| `preview-api.yml` | `apps/api/**`, `pillars/**`, `packages/**` | **ACA** ephemeral `ssd-pocpk-aca-pr-<n>-ae` |
| `deploy-web.yml` | same as ci-web, **`push` `main`** | SWA **production** via OIDC Ã¢â€ â€™ KV |
| `deploy-api.yml` | same as ci-api, **`push` `main`** (+ `workflow_dispatch`) | Nest Ã¢â€ â€™ App Service **B1** via OIDC (prebuilt `dist`; Oryx off) |
| `deploy-marketing.yml` | `apps/marketing/**`, **`push` `main`** | Marketing SWA production via OIDC Ã¢â€ â€™ KV |
