...
- **API zip deploy:** keep `SCM_DO_BUILD_DURING_DEPLOYMENT=false` + `ENABLE_ORYX_BUILD=false` on the web app (Bicep). Do **not** change app settings in the same job as zip deploy (SCM restart aborts deploy). Package with `pnpm --filter @poc-plattform-kit/api deploy --prod` (lean; do **not** zip the whole monorepo `node_modules` — ~746MB → Kudu **504** on B1), then `az webapp deploy --type zip`.
