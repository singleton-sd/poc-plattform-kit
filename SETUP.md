...
5. **Human merge only** â€” agents never merge or review other agents' work. Connected review bots leave PR comments; the human validates the test plan and merges.
...
- [ ] Implementer: pick tickets in **READY FOR AI** â†’ claim via `scripts/clickup.ps1 claim` â†’ **IN PROGRESS** â†’ PR â†’ **PR hygiene** (CI + mergeable) â†’ **READY FOR REVIEW**
...
- [ ] Assignment / Claim Token only when claiming work â€” not when browsing
- [x] Merge automation: merged task branches set ClickUp to **COMPLETE** via OIDC â†’ Key Vault (`clickup-api-token`) â†’ ClickUp REST
...
PR / preview URLs stay on Azure defaults (`*.azurestaticapps.net`, ACA preview hostnames) â€” no custom preview domains. API CORS / Auth.js redirects allow this repoâ€™s SWA instance prefixes (`https://kind-rock-0f409fe00*.azurestaticapps.net`, marketing SWA likewise) via `CORS_ORIGINS` / App Config `app:cors:origins` (see [docs/sso.md](./docs/sso.md)). Entra Auth.js callback stays on the API host; MSAL SPA redirect URIs need exact preview origins (no Entra wildcard).
...
**Entra / Auth.js (App Config â†’ Nest env):** plain `app:azureAd:clientId` / `tenantId` / `apiAudience`; KV refs `secret:auth-secret` â†’ `AUTH_SECRET`, `secret:azure-ad-client-secret` â†’ `AZURE_AD_CLIENT_SECRET`. Do not put these secrets on App Service app settings.
...
**Forward Email â€” create secret + App Config (ops; do not commit values):**
...
az keyvault secret set `
  ...
  --file -   # paste token, Enter, then Ctrl+Z Enter on Windows â€” or use --value only in a private session
...
- [x] App Service `DATABASE_URL` via `@Microsoft.KeyVault(SecretUri=â€¦/database-url/)` + MI (Bicep; not in `deploy-api.yml`)
- [ ] Nest / SWA / ACA: App Configuration provider bootstrap (follow-up â€” map `app:*` / `secret:*`)
...
| `chromatic.yml` | `apps/web/**`, `packages/**` | Storybook visual regression via OIDC â†’ KV `chromatic-project-token` |
| `playwright.yml` | `apps/web/**`, `packages/**` | Chromium public journeys + bounded failure artifacts |
| `preview-marketing.yml` | `apps/marketing/**` | Marketing SWA **PR preview** via OIDC â†’ KV (`apps/marketing/dist`) |
...
| `deploy-marketing.yml` | `apps/marketing/**`, **`push` `main`** | Astro build â†’ Marketing SWA production (`apps/marketing/dist`) via OIDC â†’ KV |
