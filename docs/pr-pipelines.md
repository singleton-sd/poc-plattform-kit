...
| `release.yml` | push to **`main`** (skipped for `chore: Release` commits) | Path-aware bumps; push `chore/release-package-versions`; open/merge PR; push tags; dispatch deploy-api / deploy-web |

**Shared packages:** changes under `packages/**` run **both** `ci-web` and `ci-api`. FE-only PRs skip API CI; API/pillar-only PRs skip web CI. On **`main`**, `release.yml` bumps versions for changed packages (conventional commits: `fix`→patch, `feat`→minor, `BREAKING CHANGE`→major; cascades api/web when `packages/**` / `pillars/**` change). It pushes an ephemeral **`chore/release-package-versions`** branch (main is PR-only), opens/merges that PR when CI is green, pushes semver tags, then **dispatches** matching **deploy-*** workflows via `workflow_dispatch` on **`main`**. A plain `GITHUB_TOKEN` push does **not** start other workflows — do not rely on the push event alone.
