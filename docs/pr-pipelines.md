...
- After deploy: `scripts/entra-spa-preview-redirect.sh add --origin https://<fqdn>` registers the preview origin as an Entra **SPA** redirect URI (MSAL). On PR `closed`, the same script removes that origin, then the Container App and ACR tags are deleted. Requires Graph `Application.ReadWrite.OwnedBy` + ownership on the Entra app (see `docs/sso.md`); missing rights soft-fail.
