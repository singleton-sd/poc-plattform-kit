...
1. **PR open/sync** (paths `apps/api/**`, `pillars/**`, `packages/**`):
   ...
   - Build `apps/api/Dockerfile` with `--build-arg PREVIEW_SEED_SCENARIOS=...` — every PR preview gets its **own isolated, disposable SQLite database**, seeded and verified at build time, baked into the image as an immutable template. It never resolves or mutates the shared PostgreSQL database (Neon PoC or Azure Flexible Server via Key Vault `database-url`).
