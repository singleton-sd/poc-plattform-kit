...
.DESCRIPTION
  ...
  GitHub Actions secrets â€” pipelines use OIDC â†’ Azure â†’ KV/App Config.
...
# Locked public hostnames (DNS in AWS Route53 â†’ Azure CNAMEs)
...
$envLines = @(
  '# Generated by infra/deploy.ps1 â€” DO NOT COMMIT',
...
# SWA deployment tokens â†’ KV only (never GitHub Secrets)
...
if (-not (Test-Path $envExample)) {
  Write-Host 'Note: .env.example missing â€” create from template in repo.'
}
...
Next steps:
  1. AWS Route53: CNAME marketing/app/api hostnames â†’ Azure defaults (+ TXT validation)
  2. Bind custom domains + managed certs on SWAs and App Service (B1)
  3. Entra app registration (SPA + API) â€” secrets in Key Vault; config in App Config
