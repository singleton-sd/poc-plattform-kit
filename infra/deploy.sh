...
set_kv_secret() {
  ...
  printf '%s' "$value" >"$tmp"
  chmod 600 "$tmp"
  if az keyvault secret set --vault-name "$vault" --name "$name" --file "$tmp" -o none; then
    echo "  set $name"
  else
...
if [[ "$rg_exists" == "false" ]]; then
  if [[ "$WHAT_IF" -eq 1 ]]; then
    die "Resource group $RESOURCE_GROUP missing. Create it (run without --what-if) before what-if."
  fi
...
umask 077
tmp_env="$(mktemp "${TMPDIR:-/tmp}/pocpk-env.XXXXXX")"
trap 'rm -f "$tmp_env"' EXIT
# Quoted heredoc: never expand $ in passwords. DATABASE_URL* filled via upsert below.
cat >"$tmp_env" <<'EOF'
...
# Fill non-secret / known values without shell-expanding URL passwords
upsert_dotenv_key "$tmp_env" AZURE_SUBSCRIPTION_ID "$SUBSCRIPTION_ID"
upsert_dotenv_key "$tmp_env" AZURE_TENANT_ID "$tenant_id"
upsert_dotenv_key "$tmp_env" AZURE_RESOURCE_GROUP "$RESOURCE_GROUP"
upsert_dotenv_key "$tmp_env" AZURE_LOCATION "$LOCATION"
upsert_dotenv_key "$tmp_env" AZURE_APP_SERVICE_NAME "$WEB_APP_NAME"
upsert_dotenv_key "$tmp_env" AZURE_APP_SERVICE_URL "$PUBLIC_API_URL"
upsert_dotenv_key "$tmp_env" AZURE_STATIC_WEB_APP_NAME "$SWA_NAME"
upsert_dotenv_key "$tmp_env" AZURE_STATIC_WEB_APP_URL "$PUBLIC_APP_URL"
upsert_dotenv_key "$tmp_env" AZURE_MARKETING_SWA_NAME "$MARKETING_SWA_NAME"
upsert_dotenv_key "$tmp_env" AZURE_MARKETING_URL "$PUBLIC_MARKETING_URL"
upsert_dotenv_key "$tmp_env" AZURE_SERVICEBUS_NAMESPACE "$SB_NS"
upsert_dotenv_key "$tmp_env" NEXT_PUBLIC_API_BASE_URL "$PUBLIC_API_URL"
upsert_dotenv_key "$tmp_env" CORS_ORIGINS "$CORS_ORIGINS"
upsert_dotenv_key "$tmp_env" AUTH_URL "$PUBLIC_API_URL"
if [[ -n "${database_url:-}" ]]; then
  upsert_dotenv_key "$tmp_env" DATABASE_URL "$database_url"
fi
if [[ -n "${database_url_unpooled:-}" ]]; then
  upsert_dotenv_key "$tmp_env" DATABASE_URL_UNPOOLED "$database_url_unpooled"
fi
...
if [[ -f "$ENV_FILE" ]]; then
  for key in AUTH_SECRET AUTH_URL AUTH_COOKIE_DOMAIN AZURE_AD_CLIENT_ID AZURE_AD_CLIENT_SECRET \
    ...
    if existing="$(read_dotenv_value "$ENV_FILE" "$key" 2>/dev/null)"; then
      ...
      upsert_dotenv_key "$tmp_env" "$key" "$existing"
    fi
  done
fi
