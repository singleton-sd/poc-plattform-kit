# AGENTS.md Ã¢â‚¬â€ poc-plattform-kit
...
5. If **Claim Token** â‰  `claimToken` â†’ abort (lost race); pick a different ticket.
...
1. **Implementer** runs the exclusive claim protocol on **READY FOR AI** â†’ implement in a **git worktree** â†’ open PR â†’ run **PR hygiene (implementer)** â†’ comment PR URL + CI status on ticket â†’ clear **Claim Token** â†’ set **READY FOR REVIEW**.
2. **Reviewer** (different AI) runs the exclusive claim protocol on **READY FOR REVIEW** â†’ review PR in a **worktree** â†’ run **PR hygiene (reviewer)** â†’ post review **comments** â†’ clear **Claim Token** â†’ set **READY FOR HUMAN** only if hygiene passes. If the implementer must stay visible, comment their identity on the ticket before/when reassigning.
...
2. `gh pr view --json mergeable,mergeStateStatus` Ã¢â€ â€™ must be `MERGEABLE` / not `DIRTY`.
3. If dirty: merge/rebase `main` in the worktree, resolve conflicts, push, re-check CI.
4. Comment ClickUp with PR URL + CI status.
5. Own green CI before handoff; after conflict fixes or follow-up commits, re-run CI before re-handing off. Env/Entra blockers (e.g. AADSTS700213): comment on ClickUp and stop Ã¢â‚¬â€ do not spin. Prefer current Node pin (24); do not default to `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION`.

#### Reviewer (before READY FOR HUMAN)

1. Re-fetch PR tip; confirm `mergeable` still clean.
2. Confirm required checks green on tip.
3. Pull **all** feedback (Bugbot does not appear in Cursor chat Ã¢â‚¬â€ treat it as a GitHub commenter):
...
Branch protection must require **CI status checks** + **human merge**, but **must not** require approving reviews. When the AI reviewer shares the PR authorÃ¢â‚¬â„¢s GitHub identity, reviews are **comments only** (never Ã¢â‚¬Å“ApproveÃ¢â‚¬Â). See `SETUP.md`.
...
Pillars (no cross-pillar DB joins or write HTTP): **Tenant**, **SingleSignOn**, **Subscriptions**, **Contact**, **Support**, **Audit**, **Reporting**, **Permissions** (OpenFGA Ã¢â‚¬â€ see Architecture Doc), **Notifications**.

- Messaging: Azure Service Bus (topics = events, queues = jobs)
- Mutations: same transaction Ã¢â€ â€™ entity + **local Audit** + **Outbox** (when others must be notified)
...
- AuthZ (fine-grained): **Permissions** pillar Ã¢â‚¬â€ `Check(subject, action, resource)`; **OpenFGA** (Zanzibar/ReBAC) on **Azure Container Apps Consumption**. Azure has no first-class app-data authZ for domain items. Other pillars call Permissions (sync HTTP or cache); never embed authZ rules in Contact/etc. Optional denial events Ã¢â€ â€™ Audit.
- Outbound messaging: **Notifications** pillar Ã¢â‚¬â€ email (Forward Email API), SMS (android-sms-gateway), WhatsApp (Meta Cloud API default; adapter swappable). Consumes domain events + queue `notifications.send`; publishes `notification.sent` / `notification.failed` on `notifications.events`.
...
- **CI/CD:** GitHub Actions **OIDC** Ã¢â€ â€™ Azure Ã¢â€ â€™ Key Vault / App Config (no deploy tokens or connection strings in GitHub Secrets)
...
- **CI (GitHub Actions):** OIDC login using repo **Variables** `AZURE_CLIENT_ID` / `AZURE_TENANT_ID` / `AZURE_SUBSCRIPTION_ID` (IDs only) Ã¢â€ â€™ `az keyvault secret show` or App Config at job runtime. **Never** put `AZURE_STATIC_WEB_APPS_API_TOKEN`, `AZURE_CREDENTIALS`, or other secrets in GitHub Secrets.
...
| --- | --- | --- | --- |
| `apps/web/**` | `ci-web.yml` | SWA PR preview (`preview-web.yml`, Free) via OIDC Ã¢â€ â€™ KV | `deploy-web.yml` Ã¢â€ â€™ SWA production |
| `apps/api/**`, `pillars/**` | `ci-api.yml` | Path B ACA (`preview-api.yml`) via OIDC Ã¢â€ â€™ KV | `deploy-api.yml` Ã¢â€ â€™ App Service B1 |
| `apps/marketing/**` | `ci-web.yml` (marketing filter) | Ã¢â‚¬â€ (no PR preview) | `deploy-marketing.yml` Ã¢â€ â€™ marketing SWA |
...
- ACA auth: OIDC Variables only Ã¢â‚¬â€ `AZURE_CLIENT_ID` / `AZURE_TENANT_ID` / `AZURE_SUBSCRIPTION_ID` (no `AZURE_CREDENTIALS`).
...
- **`pnpm sync:skills` is Windows-only** (PowerShell); skip on Linux Ã¢â‚¬â€ skills are already committed under `.cursor/skills/`.
