...
5. Own green CI before handoff; after conflict fixes or follow-up commits, re-run CI before re-handing off. Env/Entra blockers (e.g. AADSTS700213): comment on ClickUp and stop â€” do not spin. Prefer current Node pin (24); do not default to `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION`.

#### Reviewer (before READY FOR HUMAN)

1. Re-fetch PR tip; confirm `mergeable` still clean.
2. Confirm required checks green on tip.
3. Pull **all** feedback (Bugbot does not appear in Cursor chat â€” treat it as a GitHub commenter):
...
Branch protection must require **CI status checks** + **human merge**, but **must not** require approving reviews. When the AI reviewer shares the PR authorâ€™s GitHub identity, reviews are **comments only** (never â€œApproveâ€). See `SETUP.md`.
...
| --- | --- | --- | --- |
| `apps/web/**` | `ci-web.yml` | SWA PR preview (`preview-web.yml`, Free) via OIDC â†’ KV | `deploy-web.yml` â†’ SWA production |
| `apps/api/**`, `pillars/**` | `ci-api.yml` | Path B ACA (`preview-api.yml`) via OIDC â†’ KV | `deploy-api.yml` â†’ App Service B1 |
| `apps/marketing/**` | `ci-web.yml` (marketing filter) | â€” (no PR preview) | `deploy-marketing.yml` â†’ marketing SWA |
